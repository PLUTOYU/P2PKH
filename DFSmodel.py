...
In reach model using DFS tranverse function
builder:shanghengyu
'''
...
class Reach:
    ...
    counterInverse = {}
    newTrans = {}
    newTransFunction = {} #Store the set of state-to-counter in the migration function
    ...
    def start(self):
        ...
        # Implement path counter through deep first search traversal
        final_state = self.tree(self.final_state)



    # Reflect the state in the counter and update the counter value of the counter to construct a stack
    def tree(self, final_state):  # {(0, 0, 1): [('0', '1', '3', '4')]}
        ...
        #Detect Reflexive Record Reflexive
        ...
        for event in transFunction[subCounterAndState[1][-1]]:
            ...
            if subCounterAndState[0][0] >= k1 and subCounterAndState[0][1] >= k2:
                ...
                for state in transFunction[subCounterAndState[1][-1]][event]:
                    if state in subCounterAndState[1]:  # cut ring
    ...
    # at each counter ,it eed to check if the state exists reverse transition funtcion
